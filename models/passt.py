...
class Attention(nn.Module):
    ...
    def forward(self, x, attn_mask=None, attn_penalty=None):
        B, N, C = x.shape
        # (qkv, B, head, N, dim)
        ...
        # https://github.com/sephiroce/srf/blob/master/tfsr/model/attention.py#L80
        if attn_penalty is not None:
            attn += torch.log(1 + attn_penalty) * -1
        # https://github.com/google-research/bigbird/blob/master/bigbird/core/attention.py#L805
        if attn_mask is not None:
            # Since attn_mask is 1.0 for positions we want to attend and 0.0 for
            # masked positions, this operation will create a tensor which is 0.0 for
            # positions we want to attend and -10000.0 for masked positions.
            adder = ~attn_mask * -10000.0
            # plt.imshow(attn_mask.cpu().numpy(), cmap='hot', interpolation='nearest')
            # plt.show()
...
class SparseAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, block_lens:list):
        """
        block 0 is global block
        """
        B, N, C = x.shape
        # (qkv, B, head, N, dim)
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        q_splitted = torch.split(q, block_lens, dim=2)
        k_splitted = torch.split(k, block_lens, dim=2)
        v_splitted = torch.split(v, block_lens, dim=2)
        
        # attn = (q @ k.transpose(-2, -1)) * self.scale   # time consuming

        local_block_idxs = [ i for i in range(1, len(q_splitted)) ]
        attn_splitted = [ None ] * len(q_splitted)

        # global
        attn_splitted[0] = (q_splitted[0] @ k.transpose(-2, -1)) * self.scale
        # local
        for idx in local_block_idxs:
            attn_splitted[idx] = (q_splitted[idx] @ torch.cat([k_splitted[0], k_splitted[idx]], dim=2).transpose(-2, -1)) * self.scale
        
        # https://github.com/sephiroce/srf/blob/master/tfsr/model/attention.py#L80
        # (B, head, N, N)
        # attn = attn.softmax(dim=-1)
        # attn = self.attn_drop(attn)
        for idx in range(len(q_splitted)):
            attn_splitted[idx] = attn_splitted[idx].softmax(dim=-1)  # type: ignore
            attn_splitted[idx] = self.attn_drop(attn_splitted[idx])

        # (B, head, N, dim) --transpose--> (B, N, head, dim) --reshape--> (B, N, C) 
        # x = (attn @ v).transpose(1, 2).reshape(B, N, C) # time consuming
        x_splitted = [ None ] * len(q_splitted)
        # global
        x_splitted[0] = attn_splitted[0] @ v
        # local
        for idx in local_block_idxs:
            x_splitted[idx] = attn_splitted[idx] @ torch.cat([v_splitted[0], v_splitted[idx]], dim=2)
        x = torch.cat(x_splitted, dim=2).transpose(1, 2).reshape(B, N, C)
        # assert x.shape[1] == N
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, rf_norm_t='', global_tokens_num=2,
                 attention_penalty_width=297, stripe_width=99, att_penalty_scale=1.0):
        super().__init__()
        self.norm1 = norm_layer(dim)
        if "sparse_row" in rf_norm_t:
            # can load weights automatically
            self.attn = SparseAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        else:
            self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        if rf_norm_t == 'high_low_branch' or rf_norm_t == 'high_low_branch_v2':
            self.norm1_2 = norm_layer(dim)
            self.attn2 = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        if rf_norm_t == 'high_low_branch_v2':
            self.mlp2 = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
            self.norm2_2 = norm_layer(dim)
        self.rf_norm_t = rf_norm_t
        self.global_tokens_num = global_tokens_num
        self.attention_penalty_width = attention_penalty_width
        self.stripe_width = stripe_width
        self.att_penalty_scale = att_penalty_scale

    @staticmethod
    def create_attention_penalty(seq_len, global_tokens_num, attention_penalty_width, stripe_width, att_penalty_scale, device):
        enc_att_penalty = torch.ones([seq_len + global_tokens_num, seq_len + global_tokens_num], device=device)
        enc_att_penalty[:global_tokens_num, :] = 0
        enc_att_penalty[:, :global_tokens_num] = 0

        accum = torch.zeros([seq_len + global_tokens_num, seq_len + global_tokens_num], device=device)
        for i in range(attention_penalty_width, seq_len, stripe_width):
            accum += torch.triu(enc_att_penalty, i) + torch.tril(enc_att_penalty, -i)

        # plt.imshow(accum.cpu().numpy(), cmap='hot', interpolation='nearest')
        # plt.show()
        # plt.savefig('temp.png')

        return accum * att_penalty_scale

    def forward(self, input):
        if self.rf_norm_t == 'high_low_branch_v2':
            x1, x2 = input["x1"], input["x2"]
            x1 = self.drop_path(self.attn(self.norm1(x1)))
            x2 = self.drop_path(self.attn2(self.norm1_2(x2)))

            x1 = x1 + self.drop_path(self.mlp(self.norm2(x1)))
            x2 = x2 + self.drop_path(self.mlp2(self.norm2_2(x2)))
            return {"x1": x1, "x2": x2}
        elif "sparse_row" in self.rf_norm_t:
            x, block_lens = input
            x = x + self.drop_path(self.attn(self.norm1(x), block_lens=block_lens))
            ...
            if isinstance(input, tuple):
                ...
                x, attn_mask = input, None
            if self.rf_norm_t == 'high_low_branch':
                l = x.shape[1]
                x1 = self.drop_path(self.attn(self.norm1(x[:, :l//2, :]), attn_mask=attn_mask))
                x2 = self.drop_path(self.attn2(self.norm1_2(x[:, l//2:, :]), attn_mask=attn_mask))
                x = x + torch.cat([x1, x2], dim=1)
            else:
                if self.rf_norm_t == "attn_penalty":
                    penalty = self.create_attention_penalty(x.shape[1] - self.global_tokens_num, self.global_tokens_num, self.attention_penalty_width, self.stripe_width, self.att_penalty_scale, device = x.device)
                else:
                    penalty = None
                x = x + self.drop_path(self.attn(self.norm1(x), attn_mask=attn_mask, attn_penalty=penalty))
...
class PaSST(nn.Module):
    ...
    def __init__(self, u_patchout=0, s_patchout_t=0, s_patchout_f=0, img_size=(128, 998), patch_size=16, stride=16,
                 ...
                 act_layer=None, weight_init='', rf_norm_t='', global_tokens_num=2,
                 attention_penalty_width=297, stripe_width=99, att_penalty_scale=1.0):
        """
        Args:
        ...
        if global_tokens_num > 2:
            if self.dist_token is None:
                raise NotImplementedError
            self.global_tokens = nn.Parameter(torch.zeros(1, global_tokens_num, embed_dim))
        # PaSST
        ...
        if global_tokens_num > 2:
            self.new_pos_embed_extended = nn.Parameter(torch.zeros(1, global_tokens_num, embed_dim))
        self.global_tokens_num = global_tokens_num
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, rf_norm_t=rf_norm_t,
                global_tokens_num=global_tokens_num,attention_penalty_width=attention_penalty_width, stripe_width=stripe_width, att_penalty_scale=att_penalty_scale)
    ...
    def forward_features(self, x):
        ...
        if self.rf_norm_t == 'high_low_branch' and (self.s_patchout_t or self.s_patchout_f or self.u_patchout):
            raise RuntimeError("high_low_branch and patchout are not compatible for now")
        
        pre_mask = torch.zeros([1,1])   # dummy
        _, _, row, col = x.shape
        if self.rf_norm_t == 'high_low' or 'per_row' in self.rf_norm_t or (isinstance(self.rf_norm_t, str) and  'overlap' in self.rf_norm_t):
            if self.rf_norm_t == 'high_low':
                pre_mask = torch.empty([row,col], dtype=torch.int64)
                pre_mask[:row//2, :] = 0
                pre_mask[row//2:, :] = 1
            elif 'per_row' in self.rf_norm_t or (isinstance(self.rf_norm_t, str) and 'overlap' in self.rf_norm_t):
                ops = self.rf_norm_t.split('_')
                if 'per_row' in self.rf_norm_t and len(ops) == 3:
                    l = int(ops[2])
                    pre_mask = torch.empty([row,col], dtype=torch.int64)
                    for i, j in enumerate(range(0, row, l)):
                        pre_mask[j:j+l, :] = i
                else:
                    pre_mask = torch.arange(0,row).unsqueeze(-1).expand(-1,col)
            else:
                raise NotImplementedError
        ...
        if self.training and self.s_patchout_t:
            ...
            x = x[:, :, :, random_indices]
            if self.rf_norm_t == 'high_low' or 'per_row' in self.rf_norm_t or (isinstance(self.rf_norm_t, str) and 'overlap' in self.rf_norm_t):
                pre_mask = pre_mask[:, random_indices]
        ...
        # (BNC) flatten的patch，对于flatten的patch做patchout
        ...
        if self.training and self.u_patchout:
            ...
            # assert pre_mask.shape[0] == x.shape[1]
            x = x[:, random_indices, :]
            if self.rf_norm_t == 'high_low' or 'per_row' in self.rf_norm_t or (isinstance(self.rf_norm_t, str) and 'overlap' in self.rf_norm_t):
                pre_mask = pre_mask[random_indices]
        ...
        if self.rf_norm_t == 'high_low' or 'per_row' in self.rf_norm_t or (isinstance(self.rf_norm_t, str) and 'overlap' in self.rf_norm_t):
            _, mask_lens = torch.unique(pre_mask, return_counts=True)
            seq_len = x.shape[1]
        mask = None
        ...
        cls_tokens = self.cls_token.expand(B_dim, -1, -1) + self.new_pos_embed[:, :1, :]    # self.new_pos_embed[:, :1, :]对应cls_token, self.new_pos_embed[:, 1:, :]对应dist_token
        if first_RUN: print(" self.cls_tokens.shape", cls_tokens.shape)
        seq_len = x.shape[1]
        if self.rf_norm_t == "high_low_branch_v2":
            if self.s_patchout_f or self.s_patchout_t or self.u_patchout:
                raise NotImplementedError
            if self.dist_token is None:
                raise Exception("dist_token should not be None")
            dist_token = self.dist_token.expand(B_dim, -1, -1) + self.new_pos_embed[:, 1:, :]
            x1 = x[:, :seq_len//2, :]
            x1 = torch.cat((cls_tokens, dist_token, x1), dim=1)
            x2 = x[:, seq_len//2:, :]
            x2 = torch.cat((cls_tokens, dist_token, x2), dim=1)

        elif self.dist_token is None:
            x = torch.cat((cls_tokens, x), dim=1)
            raise Exception("Not implemented by now")
            if self.rf_norm_t == 'high_low' or self.rf_norm_t == 'per_row' or (isinstance(self.rf_norm_t, str) and 'overlap' in self.rf_norm_t):
                mask = torch.zeros((seq_len+1, seq_len+1), device=x.device, dtype=torch.bool)
                mask[0,:] = True
                mask[:,0] = True
                cum = 0
                for l in mask_lens:
                    mask[cum+1:cum+2*l+1, cum+1:cum+2*l+1] = True
                    cum += l
                # assert cum+1 == mask.shape[0] and cum+1 == mask.shape[1]
                # assert torch.sum(mask) == sum([l**2 for l in mask_lens]) + seq_len*2 + 1
        else:
            if self.global_tokens_num > 2:
                global_tokens = self.global_tokens.expand(B_dim, -1, -1) + self.new_pos_embed_extended
                if first_RUN: print(" global_tokens.shape", global_tokens.shape)
                x = torch.cat((global_tokens, x), dim=1)
            else:
                dist_token = self.dist_token.expand(B_dim, -1, -1) + self.new_pos_embed[:, 1:, :]
                if first_RUN: print(" self.dist_token.shape", dist_token.shape)
                x = torch.cat((cls_tokens, dist_token, x), dim=1)
            if self.rf_norm_t == 'high_low' or 'per_row' in self.rf_norm_t or 'overlap' in self.rf_norm_t:
                mask = torch.zeros((seq_len+self.global_tokens_num, seq_len+self.global_tokens_num), device=x.device, dtype=torch.bool)
                if self.global_tokens_num == 2:
                    mask[[0,1],:] = True
                    mask[:,[0,1]] = True
                else:
                    for i in range(self.global_tokens_num):
                        seq_len = x.shape[1]
                        random_indices = torch.randperm(seq_len)[:seq_len - int(seq_len * 0.15)].sort().values
                        mask[i,random_indices] = True
                        mask[random_indices,i] = True
                cum = 0
                if 'per_row' in self.rf_norm_t or self.rf_norm_t == 'high_low' or 'row_overlap' in self.rf_norm_t:
                    if 'per_row' in self.rf_norm_t or self.rf_norm_t == 'high_low':
                        overlap_factor = 1
                    elif 'row_overlap' in self.rf_norm_t:
                        ops = self.rf_norm_t.split('_')
                        if len(ops) == 3:
                            overlap_factor = int(ops[2])
                        else:
                            overlap_factor = 2
                    else:
                        raise NotImplementedError
                    for l in mask_lens:
                        mask[cum+self.global_tokens_num:cum+overlap_factor*l+self.global_tokens_num, cum+self.global_tokens_num:cum+overlap_factor*l+self.global_tokens_num] = True
                        cum += l
                elif 'token_overlap' in self.rf_norm_t:
                    ops = self.rf_norm_t.split('_')
                    if len(ops) == 4:
                        i = self.global_tokens_num
                        total = mask.shape[0]
                        l = int(ops[2])
                        step = max(int(l * float(ops[3])), 1)
                        while i < total:
                            mask[i:i+l, i:i+l] = True
                            i += step
                    else:
                        for i in range(mask.shape[0] - self.global_tokens_num):
                            l = int(ops[2]) if len(ops) == 3 else mask_lens[0]
                            mask[i+self.global_tokens_num:i+l+self.global_tokens_num, i+self.global_tokens_num:i+l+self.global_tokens_num] = True
                else:
                    raise NotImplementedError

                # assert cum+2 == mask.shape[0] and cum+2 == mask.shape[1]
                # assert torch.sum(mask) == sum([l**2 for l in mask_lens]) + seq_len*4 + 4
        if self.rf_norm_t == "high_low_branch_v2":
            if first_RUN: print(f" final sequence x1: {x1.shape}, x2: {x2.shape}")
            x1 = self.pos_drop(x1)
            x2 = self.pos_drop(x2)
            # x, _ = self.blocks((x, mask))
            d = self.blocks({"x1": x1, "x2": x2})
            x1, x2 = d["x1"], d["x2"]
            if first_RUN: print(f" after {len(self.blocks)} atten blocks x1: {x1.shape}, x2: {x2.shape}")
            x1 = self.norm(x1)
            x2 = self.norm2(x2)
            ...
            if self.u_patchout or self.s_patchout_f or self.s_patchout_t:
                raise NotImplementedError
            if first_RUN: print(" final sequence x", x.shape)
            block_lens = [ self.global_tokens_num ]
            ops = self.rf_norm_t.split('_')
            block_len = int(ops[2]) * col
            block_lens += [ block_len ] * (row * col // block_len)
            if row * col % block_len > 0:
                block_lens += [row * col % block_len]
            # assert sum(block_lens) == row * col + self.global_tokens_num
            x = self.pos_drop(x)
            x, _ = self.blocks((x, block_lens))
            if first_RUN: print(f" after {len(self.blocks)} atten blocks x", x.shape)
            x = self.norm(x)

            if self.global_tokens_num > 2:
                return torch.mean(x[:,:self.global_tokens_num], dim=1)
            elif self.dist_token is None:
...
@model_ing.command
def fix_high_low_attnweights(model):
    for block in model.blocks:
        block.attn2.load_state_dict(block.attn.state_dict()) 
        block.norm1_2.load_state_dict(block.norm1.state_dict()) 
        if hasattr(block, 'mlp2'):
            block.mlp2.load_state_dict(block.mlp.state_dict())
        if hasattr(block, 'norm2_2'):
            block.norm2_2.load_state_dict(block.norm2.state_dict()) 
    if hasattr(model,'norm2'):
        model.norm2.load_state_dict(model.norm.state_dict())
    return model

@model_ing.command
def fix_global_tokens(model, global_tokens_num):
    old_size = model.new_pos_embed.shape[1]
    if global_tokens_num <= old_size or global_tokens_num % old_size > 0:
        raise ValueError(f"global_tokens_num should be a multiple of {old_size} and larger than {old_size}")
    with torch.no_grad():
        model.new_pos_embed_extended.copy_(model.new_pos_embed.repeat(1, global_tokens_num // old_size, 1))
        model.global_tokens.copy_(torch.cat([model.cls_token, model.dist_token], dim=1).repeat(1, global_tokens_num // old_size, 1))
    return model

@model_ing.command
def get_model(arch="passt_s_swa_p16_128_ap476", pretrained=True, n_classes=527, in_channels=1, fstride=10,
              tstride=10,
              input_fdim=128, input_tdim=998, u_patchout=0, s_patchout_t=0, s_patchout_f=0, rf_norm_t='',
              global_tokens_num=2, attention_penalty_width=0, stripe_width=0, att_penalty_scale=0.0
              ):
    """
    ...
    :param rf_norm_t: normalize the receptive field of the model in time: 'per_row', 'high_low', 'high_low_branch', 'row_overlap', 'token_overlap'
    :param global_tokens_num: number of global tokens to add to the model, default: 2
    ...
    model = model_func(pretrained=pretrained, num_classes=n_classes, in_chans=in_channels,
                       ...
                       s_patchout_t=s_patchout_t, s_patchout_f=s_patchout_f, rf_norm_t=rf_norm_t, global_tokens_num=global_tokens_num,
                       attention_penalty_width=attention_penalty_width, stripe_width=stripe_width, att_penalty_scale=att_penalty_scale)
    model = fix_embedding_layer(model)
    model = lighten_model(model)
    if rf_norm_t == 'high_low_branch' or rf_norm_t == 'high_low_branch_v2':
        model = fix_high_low_attnweights(model)
    if global_tokens_num > 2:
        model = fix_global_tokens(model, global_tokens_num)
    print(model)
    return model
